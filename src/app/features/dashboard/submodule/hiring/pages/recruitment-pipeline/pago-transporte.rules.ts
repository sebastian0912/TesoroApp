...
/**
 * Lee el valor tolerando el casing con el que llega el contrato.
 * El serializer expone `Ccentro_de_costos` (C mayúscula), pero el adaptador
 * `snakeDeep` del servicio lo baja a `ccentro_de_costos` al aplanar el candidato,
 * así que en el pipeline el contrato llega en minúscula. Sin esta tolerancia,
 * "Centro de costos" saldría SIEMPRE como faltante y bloquearía la generación.
 */
function leerCampo(contrato: any, campo: string): any {
  const v = contrato[campo];
  if (v !== undefined) return v;
  return contrato[campo.toLowerCase()];
}
...
export function faltantesDePagoTransporte(contrato: any): string[] {
  ...
  const faltan = CAMPOS_PAGO_TRANSPORTE
    .filter(({ campo }) => vacio(leerCampo(contrato, campo)))
