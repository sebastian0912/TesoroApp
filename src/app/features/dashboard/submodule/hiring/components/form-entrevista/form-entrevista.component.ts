import { ChangeDetectionStrategy, Component, ElementRef, effect, inject, input, OnInit, AfterViewInit, ViewChild } from '@angular/core';
import { FormArray, FormBuilder, FormControl, FormGroup, ValidatorFn, AbstractControl, ValidationErrors, Validators } from '@angular/forms';
...
import { GestionParametrizacionService, CatalogValue } from '../../../users/services/gestion-parametrizacion/gestion-parametrizacion.service';

@Component({
  selector: 'app-form-entrevista',
  imports: [MatStepperModule, MatIconModule, MatProgressBarModule, SharedModule],
  templateUrl: './form-entrevista.component.html',
  styleUrls: ['./form-entrevista.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FormEntrevistaComponent implements OnInit, AfterViewInit {
  // ====== Inputs / Servicios ======
  candidatoSeleccionado = input<any | null>(null);
  private readonly fb = inject(FormBuilder);
  private readonly dateAdapter = inject<DateAdapter<Date>>(DateAdapter as any);
  ...
  comoSeEnteroOpciones$: Observable<CatalogValue[]> =
    this.catalogos.listDatosByTablaCodigo('CATALOGO_MARKETING', { activo: true });
  ...
  readonly otroExperienciaControl = new FormControl('', [Validators.maxLength(64)]);
  ...
  readonly oficinas = [
    ...
    'FORANEOS', 'FUNZA', 'MADRID', 'ROSAL', 'SOACHA', 'SUBA', 'TOCANCIPÁ', 'ZIPAQUIRÁ', 'BRIGADA'
  ] as const;

  // ==== Campos por paso (para validar tras patchValue) ====
  private readonly step1Fields = ['oficina', 'tipo_doc', 'numero_documento', 'fecha_expedicion', 'mpio_expedicion'];
  private readonly step2Fields = ['primer_apellido', 'primer_nombre', 'fecha_nacimiento', 'mpio_nacimiento', 'sexo', 'estado_civil'];
  private readonly step3Fields = ['barrio', 'celular', 'whatsapp', 'personas_con_quien_convive', 'hace_cuanto_vive'];
  ...
  constructor() {
    ...
    this.formVacante = this.fb.group({
      ...
      personas_con_quien_convive: [[], [Validators.required, this.minLengthArray(1)]],
    ...
    // ====== Reacciones ======
    this.ctrl('fecha_nacimiento').valueChanges.pipe(startWith(this.ctrl('fecha_nacimiento').value))
      .subscribe(() => this.setEdad());

    this.ctrl('tieneHijos').valueChanges.pipe(startWith(this.ctrl('tieneHijos').value))
      .subscribe((tiene: boolean) => this.setupHijosValidators(!!tiene));

    this.ctrl('numeroHijos').valueChanges.pipe(startWith(this.ctrl('numeroHijos').value))
      .subscribe((n: number) => this.setHijosCount(Number(n) || 0));

    this.ctrl('experienciaFlores').valueChanges.pipe(startWith(this.ctrl('experienciaFlores').value))
      .subscribe(val => {
    ...
    this.ctrl('tipoExperienciaFlores').valueChanges.pipe(startWith(this.ctrl('tipoExperienciaFlores').value))
      .subscribe(value => {
        if (value === 'OTROS') {
    ...
    // ===== Paso 7: dinámicas en tiempo real =====
    // Referenciado -> nombreReferenciado requerido si 'SI'
    this.ctrl('referenciado').valueChanges.pipe(startWith(this.ctrl('referenciado').value))
      .subscribe(v => {
        const nombreRef = this.ctrl('nombreReferenciado');
        if (v === 'SI') {
          nombreRef.setValidators([Validators.required, Validators.maxLength(120)]);
        } else {
    ...
    // aplicaObservacion -> obliga el motivo correspondiente
    this.ctrl('aplicaObservacion').valueChanges.pipe(startWith(this.ctrl('aplicaObservacion').value))
      .subscribe(v => {
        const motEsp = this.ctrl('motivoEspera');
        const motNoAp = this.ctrl('motivoNoAplica');

        if (v === 'EN_ESPERA') {
          motEsp.setValidators([Validators.required, Validators.maxLength(300)]);
          motNoAp.clearValidators(); motNoAp.setValue('', { emitEvent: false });
        } else if (v === 'NO_APLICA') {
          motNoAp.setValidators([Validators.required, Validators.maxLength(300)]);
          motEsp.clearValidators(); motEsp.setValue('', { emitEvent: false });
        } else if (v === 'APLICA') {
          motEsp.clearValidators(); motEsp.setValue('', { emitEvent: false });
          motNoAp.clearValidators(); motNoAp.setValue('', { emitEvent: false });
        } else {
  ...
  ngOnInit(): void {
    ...
    this.step5Ctrl.setValidators(this.makeValidator(['nivel', 'estudiaActualmente', 'proyeccion1Ano', 'experienciaFlores'], () => {
      const exp = this.ctrl('experienciaFlores').value === 'Sí';
      if (!exp) return true;
      if (!this.areValid(['tipoExperienciaFlores'])) return false;
      return this.ctrl('tipoExperienciaFlores').value === 'OTROS' ? this.otroExperienciaControl.valid : true;
    }));
    this.step6Ctrl.setValidators(() => (this.experienciasFA.valid ? null : { stepInvalid: true }));

    // Paso 7: validador integral (coincide con el HTML actual)
    this.step7Ctrl.setValidators(() => {
      ...
      const nombreReferenciadoOk = referenciado === 'SI' ? !!val('nombreReferenciado') : true;
  ...
  onStepChange(e: StepperSelectionEvent) {
    ...
    headers?.[e.selectedIndex]?.scrollIntoView({ behavior: 'smooth', inline: 'start', block: 'nearest' });
  }
  scrollHeaders(direction: 1 | -1) {
    ...
    if (!ctn) return;
    ctn.scrollBy({ left: Math.round(ctn.clientWidth * 0.85) * direction, behavior: 'smooth' });
  }
  ...
  get municipioCtrl() { return this.ctrl('mpio_expedicion'); }
  get lugarNacimientoCtrl() { return this.ctrl('mpio_nacimiento'); }
  ...
  private filterCities(value: string): string[] {
    ...
    const q = norm(value);
    return q ? this.allCities.filter(c => norm(c).includes(q)).slice(0, 50) : this.allCities.slice(0, 50);
  }
  onMunicipioSelected(e: MatAutocompleteSelectedEvent) { this.municipioCtrl.setValue(e.option.value); }
  onLugarNacimientoSelected(e: MatAutocompleteSelectedEvent) { this.lugarNacimientoCtrl.setValue(e.option.value); }

  // ====== Hijos ======
  get hijosFA(): FormArray { return this.formVacante.get('hijos') as FormArray; }
  ...
  private setupHijosValidators(tiene: boolean): void {
    ...
    if (tiene) {
      ...
      cuidador.clearValidators(); cuidador.setValue('', { emitEvent: false });
      num.clearValidators(); num.setValue(0, { emitEvent: false });
  ...
  // ====== Experiencias ======
  get experienciasFA(): FormArray { return this.formVacante.get('experiencias') as FormArray; }
  addExperiencia(): void { this.experienciasFA.push(this.buildExperienciaGroup(true)); this.refreshSteps(); }
  removeExperiencia(i: number): void { if (i >= this.SEED_EXP_COUNT) this.experienciasFA.removeAt(i); this.refreshSteps(); }
  private buildExperienciaGroup(required = true): FormGroup {
    const req = required ? [Validators.required, Validators.maxLength(255)] : [Validators.maxLength(255)];
    return this.fb.group({
      empresa: ['', req],
  ...
  private seedExperiencias(n = this.SEED_EXP_COUNT): void {
    while (this.experienciasFA.length < n) this.experienciasFA.push(this.buildExperienciaGroup(false));
  }

  // ====== Helpers ======
  private ctrl(name: string) { return this.formVacante.get(name)!; }
  private areValid(keys: string[]): boolean {
    return keys.every(k => { const c = this.ctrl(k); return !!c && (c.disabled || c.valid); });
  }
  private makeValidator(keys: string[], extra?: () => boolean): ValidatorFn {
    return () => (this.areValid(keys) && (extra ? extra() : true)) ? null : { stepInvalid: true };
  }
  ...
  private linkStepToControls(step: FormGroup, keys: string[]) {
    keys.forEach(k => this.ctrl(k).valueChanges.subscribe(() => {
      step.updateValueAndValidity({ onlySelf: true, emitEvent: false });
    }));
  }

  // === Forzar validación de un paso tras patchValue ===
  private forceValidateStep(stepCtrl: FormGroup, keys: string[]) {
    keys.forEach(k => this.ctrl(k).updateValueAndValidity({ emitEvent: false }));
    stepCtrl.updateValueAndValidity({ onlySelf: true, emitEvent: false });
  }

  // Público para plantillas/handlers
  refreshSteps(): void {
    ...
    this.step7Ctrl.updateValueAndValidity({ onlySelf: true, emitEvent: false });
  }
  ...
  private normalizeOffice(s: string): string {
    return s.normalize('NFD').replace(/\p{Diacritic}/gu, '').replace(/[\s-]+/g, '_').toUpperCase();
  }
    ...
    this.route.queryParamMap.subscribe(params => {
      ...
      if (match) {
        ...
        if (match === 'BRIGADA') {
          ...
          if (brig) this.ctrl('brigadaDe').setValue(brig, { emitEvent: false });
        }
      }
      this.refreshSteps();
    });
  ...
  private rellenarForm(cand: any): void {
    ...
    // 2) Validar pasos 1 y 2 inmediatamente después del patch
    this.forceValidateStep(this.step1Ctrl, this.step1Fields);
    this.forceValidateStep(this.step2Ctrl, this.step2Fields);

    // 3) Campos derivados
    this.ctrl('fecha_nacimiento').setValue(fechaNac, { emitEvent: true }); // recalcula edad

    // 4) “¿Con quién vive?” -> resolver a códigos (ASÍNCRONO)
    const rawConvive = String(cand?.vivienda?.personas_con_quien_convive ?? '');
    const tokens = rawConvive.split(',').map(s => s.trim()).filter(Boolean);
    this.conQuienViveOpciones$.pipe(take(1)).subscribe((opts) => {
      const up = (x: any) => String(x ?? '').trim().toUpperCase();
      const selectedCodes = tokens.map(tok => {
        const TK = up(tok);
    ...
    hijosArr.forEach((h: any, i: number) => {
      ...
      fg?.patchValue({ numero_de_documento: onlyDigits(h?.numero_de_documento), fecha_nac: toDate(h?.fecha_nac) }, { emitEvent: false });
    });

    // Paso 6: Experiencias (detalle)
    const exps = Array.isArray(cand?.experiencias) ? cand.experiencias : [];
    const totalCards = Math.max(exps.length, this.SEED_EXP_COUNT);
    while (this.experienciasFA.length < totalCards) this.experienciasFA.push(this.buildExperienciaGroup(false));
    while (this.experienciasFA.length > totalCards) this.experienciasFA.removeAt(this.experienciasFA.length - 1);
    ...
    for (let i = exps.length; i < totalCards; i++) {
      (this.experienciasFA.at(i) as FormGroup)?.patchValue({
        empresa: '', tiempo_trabajado: '', labores_realizadas: '', labores_principales: ''
      }, { emitEvent: false });
    }

    // 5) Forzar validación de TODOS los pasos dependientes
    this.forceValidateStep(this.step3Ctrl, this.step3Fields);
    ...
    // 6) Recalcular el formulario completo al final
  ...
  async onSubmit() {
    ...
    try {
      const f = this.formVacante.getRawValue();

      // Upsert por documento + marcar proceso.entrevistado = true
      await firstValueFrom(
        this.candidateService.upsertCandidatoByDocumentoFromForm(f, { entrevistado: true })
      );

      await Swal.fire({ icon: 'success', title: 'Listo', text: 'Datos guardados y entrevista marcada.' });
    } catch (e: any) {
      const msg = e?.error?.detail || e?.message || 'No se pudo guardar.';
