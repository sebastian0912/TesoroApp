...
export class GenerateContractingDocumentsComponent implements OnInit {
  ...
  /** Delegado: la lógica vive en contrato-texto.util.ts (probada con specs). */
  parseDateToDDMMYYYY(v: any): string {
    return parseDateToDDMMYYYY(v);
  }
  ...
  async generarContratoCompletoTrabajoTuAlianza(isBatch: boolean = false) {
    ...
    // Tokenizador extraído a contrato-texto.util.ts (lógica pura con specs):
    // une letras sueltas, NO junta dígitos ("CLL 7 4 49" sale tal cual).
    const normalizeText = normalizeTextoContrato;
